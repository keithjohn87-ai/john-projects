...
function checkout() {
    ...
    // Debug logging
    console.log('Checkout called. StripeCheckout available:', typeof StripeCheckout !== 'undefined');
    if (typeof StripeCheckout !== 'undefined') {
        console.log('processCheckout available:', typeof StripeCheckout.processCheckout);
        console.log('redirectToCheckout available:', typeof StripeCheckout.redirectToCheckout);
    }
    
    if (typeof StripeCheckout !== 'undefined' && StripeCheckout.processCheckout) {
        StripeCheckout.processCheckout();
    } else {
        // Fallback: redirect to checkout with first item
        const firstItem = cart.items[0];
        if (firstItem && typeof StripeCheckout !== 'undefined' && StripeCheckout.redirectToCheckout) {
            StripeCheckout.redirectToCheckout(firstItem.id);
        } else {
            console.error('StripeCheckout not available:', typeof StripeCheckout);
            alert('Payment system is loading. Please wait a moment and try again. If the issue persists, refresh the page.');
        }
    }
}

function processPayment() {
    // Alias for checkout function
    checkout();
}
...
function showForgotPassword() {
    // Hide login and register tabs, show forgot tab
    ...
    document.querySelector('#forgotTab form').style.display = 'block';
}
...
function handleForgotPassword(e) {
    ...
    document.querySelector('#forgotTab form').style.display = 'none';
...
// Override showTab to handle the account modal tabs properly
function showTab(tabName) {
    const loginBtn = document.getElementById('loginTabBtn');
    const registerBtn = document.getElementById('registerTabBtn');
    const forgotBtn = document.getElementById('forgotTabBtn');
    ...
    if (tabName === 'login') {
        // Show login and register tabs, hide forgot
